...
if($_POST['id']){
	...
	$upd = $db->query("UPDATE post SET `body`='{$text}', `link`='{$_POST['link']}' WHERE `pid`={$_POST['id']}");
	if($upd){
	...
	$new = $db->query("INSERT INTO post (`iid`,`created`,`body`,`link`) VALUES ({$_POST['iid']}, '$crea', '$text', '{$_POST['link']}') ");
	if($new){
